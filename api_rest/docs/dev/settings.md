...
- **`api_rest/conf/settings.py`**: Main Settings class
- **`api_rest/conf/db.py`**: Database settings
- **`api_rest/settings.py`**: Global settings instance
...
```python
from api_rest.conf import settings
...
from api_rest.conf.settings import Settings
...
```python
from api_rest.conf.cache import CacheSettings
