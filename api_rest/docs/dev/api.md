...
The FastAPI application is defined in `api_rest/www.py` and includes:

- **Automatic API documentation** at `/docs` (Swagger UI) and `/redoc` (ReDoc)
- **Static file serving** from `api_rest/static/` via the `/static/` endpoint
...
Create a new route in `api_rest/www.py`:
...
from api_rest.services.db import get_session_depends
...
Static files are served from `api_rest/static/`:
...
uvicorn api_rest.www:app --reload --host 0.0.0.0 --port 8000
...
uvicorn api_rest.www:app --host 0.0.0.0 --port 8000 --workers 4

# Or using gunicorn with uvicorn workers
gunicorn api_rest.www:app -w 4 -k uvicorn.workers.UvicornWorker
```
