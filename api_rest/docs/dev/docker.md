...
ENV APP_MODULE=api_rest.www:app
...
```yaml
volumes:
  - "./api_rest:/app/api_rest"  # Source code
...
docker build -f dockerfile.www -t api_rest-www:latest .

# Tag for registry
docker tag api_rest-www:latest ghcr.io/your-org/api_rest-www:latest

# Push to registry
docker push ghcr.io/your-org/api_rest-www:latest
```
...
FROM ghcr.io/multi-py/python-uvicorn:py3.12-slim-LATEST AS builder
...
if [ ! -z "$CREATE_TEST_DATA" ]; then
  ...
  python -m api_rest.cli test-data
fi
...
services:
  www:
    image: ghcr.io/your-org/api_rest-www:latest
    restart: always
...
# Run a command
docker-compose exec www python -m api_rest.cli version

# Check database connection
docker-compose exec www python -c "from api_rest.services.db import engine; print(engine)"
```
